...
public interface ServicioRepository extends JpaRepository<Servicio, Long> {
    ...
    List<Servicio> findAllBySitioClienteUsuarioId(Long usuarioId);

    @Query("SELECT s FROM Servicio s " +
            "WHERE s.sitio.cliente.usuario.id = :usuarioId " +
            "AND (:clienteId IS NULL OR s.sitio.cliente.id = :clienteId) " +
