...
public class ServicioService {
    ...
    public ServicioDtos.ServicioResponseDto crearServicio(Usuario usuario, ServicioDtos.ServicioRequestDto dto) {
        ...
        // Validar que el sitio pertenezca al usuario autenticado
        if (!sitio.getCliente().getUsuario().getId().equals(usuario.getId())) {
            throw new RuntimeException("No tienes permiso para registrar servicios en este sitio");
        }
    ...
    public ServicioDtos.ServicioResponseDto obtenerPorId(Usuario usuario, Long id) {
        Servicio s = obtenerServicioVerificado(usuario, id);
    ...
    public ServicioDtos.ServicioResponseDto actualizarServicio(Usuario usuario, Long id, ServicioDtos.ServicioRequestDto dto) {
        Servicio servicio = obtenerServicioVerificado(usuario, id);

        // Si se cambia el sitio asignado, validar que el nuevo sitio también pertenezca al usuario
        if (!servicio.getSitio().getId().equals(dto.sitioId())) {
            ...
            if (!nuevoSitio.getCliente().getUsuario().getId().equals(usuario.getId())) {
                throw new ResponseStatusException(HttpStatus.FORBIDDEN, "No tienes permiso para asignar este sitio");
            }
    ...
    public void subirFactura(Usuario usuario, Long servicioId, MultipartFile archivo) {
        ...
        Servicio servicio = obtenerServicioVerificado(usuario, servicioId);
    ...
    private Servicio obtenerServicioVerificado(Usuario usuario, Long servicioId) {
        Servicio servicio = servicioRepository.findById(servicioId)
                .orElseThrow(() -> new RuntimeException("Servicio no encontrado"));

        if (!servicio.getSitio().getCliente().getUsuario().getId().equals(usuario.getId())) {
            throw new RuntimeException("Acceso denegado a este servicio");
        }
        return servicio;
    }
